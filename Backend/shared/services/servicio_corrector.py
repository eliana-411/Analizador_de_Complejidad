...
from shared.services.llm_servicio import LLMServicio
...
class ServicioCorrector:
    ...
    def __init__(self):
        ...
        # Backend/shared/services/servicio_corrector.py -> Backend/data/pseudocodigos/correctos
        self.ruta_ejemplos = (
            Path(__file__).resolve().parent.parent.parent  # ✅ Fix: 3 parent en lugar de 2
            / "data"
            / "pseudocodigos"
            / "correctos"
        )
    ...
    def _cargar_base_conocimiento(self):
        ...
        print(
            f"✅ Base de conocimiento cargada: {len(self.base_conocimiento)} ejemplos correctos"
        )
    ...
    def _detectar_tipo_algoritmo(self, pseudocodigo: str) -> str:
        ...
        return (
            "recursivo"
            if "CALL" in pseudocodigo and self._es_recursivo(pseudocodigo)
            else "iterativo"
        )
    ...
    def _buscar_ejemplos_similares(
        self, errores: List[str], tipo_algoritmo: str = None
    ) -> List[Dict]:
        """
        ...
        for ejemplo in self.base_conocimiento:
            ...
            if score > 0:
                ejemplos_relevantes.append({**ejemplo, "score": score})
    ...
    def corregir(self, pseudocodigo_erroneo: str, resultado_validacion: Dict) -> Dict:
        """
        ...
        prompt = self._generar_prompt_correccion(
            pseudocodigo_erroneo, errores, ejemplos_similares
        )

        # Llamar al LLM con el contexto RAG
        try:
            llm = LLMServicio.get_llm(
                temperature=0.3
            )  # Baja temperatura para ser más preciso
...
"""
    ...
    def obtener_estadisticas_base(self) -> Dict:
        ...
        recursivos = sum(
            1 for ej in self.base_conocimiento if ej["tipo"] == "recursivo"
        )
