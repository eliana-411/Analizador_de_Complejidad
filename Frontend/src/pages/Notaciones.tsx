...
export default function Notaciones() {
  ...
  onMount(async () => {
    ...
    try {
      console.log('Analizando para notaciones asintóticas...');
      ...
      if (resultado.complejidades) {
        ...
        if (comp.mejor_caso) {
          boundsData.push({
            ...
            valor: comp.mejor_caso,
            notacion: `Ω(${comp.mejor_caso})`,
            expanded: {
        ...
        if (comp.caso_promedio) {
          boundsData.push({
            ...
            valor: comp.caso_promedio,
            notacion: `Θ(${comp.caso_promedio})`,
            expanded: {
        ...
        if (comp.peor_caso) {
          boundsData.push({
            ...
            valor: comp.peor_caso,
            notacion: `O(${comp.peor_caso})`,
            expanded: {
  ...
  return (
    <ContentContainer>
      <div class="space-y-8 animate-fade-in-up">
        <H1 class="text-center">Cotas Asintóticas - {algorithmName()}</H1>
        ...
        <Show when={!isLoading() && !error()}>
                ...
                <div class="space-y-6 font-mono text-xs text-green-300 leading-relaxed">
                  <Show when={bounds().length > 0}>
                    ...
                    <Show when={bounds().find(b => b.cota.includes('Superior'))}>
                      {(bound) => (
                        <div class="space-y-2">
                          <p class="text-green-400 font-bold text-sm">Cota Superior - {bound().valor}</p>
                    ...
                    <Show when={bounds().find(b => b.cota.includes('Ajustada'))}>
                      {(bound) => (
                        <div class="space-y-2">
                          <p class="text-green-400 font-bold text-sm">Cota Ajustada - {bound().valor}</p>
                          <div class="bg-black/40 rounded p-3 border border-green-900/50">
                            <p>∃ c₁, c₂, n₀ &gt; 0 tal que:</p>
                    ...
                    <Show when={bounds().find(b => b.cota.includes('Inferior'))}>
                      {(bound) => (
                        <div class="space-y-2">
                          <p class="text-green-400 font-bold text-sm">Cota Inferior - {bound().valor}</p>
                          <div class="bg-black/40 rounded p-3 border border-green-900/50">
                            <p>∃ c, n₀ &gt; 0 tal que:</p>
                            <p class="mt-1">T(n) ≥ c·g(n) ∀ n ≥ n₀</p>
                          </div>
          ...
          <Show when={!isLoading() && !error() && bounds().length > 0}>
            <div class="flex justify-end mt-8">
              <button
                onClick={() => navigate(`/resultados?pseudocodigo=${encodeURIComponent(searchParams.pseudocodigo || '')}`)}
