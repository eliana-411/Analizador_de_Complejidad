...
**Fecha de generación:** 06/12/2025 20:47:52  
...
**Peor Caso:**
```
E[T]=q*(7*(n+1)/2+10)+(1-q)*(7*n+5)=(7*q*n)/2+(7*q)/2+10*q+7*n-7*q*n+5-5*q=(7*n)/2-(7*q*n)/2+(7*q)/2+5*q+5
```
...
**Ecuación:** `7*k/n + 14 + 114/n`
**Método:** AnalizadorDirecto

**Pasos:**
1. 📝 Expresión: 7*k/n + 14 + 114/n
2. 
3. 🔹 ANÁLISIS DE EXPRESIÓN DIRECTA
4.    Esta NO es una recurrencia, es una fórmula cerrada
5. 
6. 🔹 PASO 1: Analizar expresión
7.    Expresión: 7*k/n + 14 + 114/n
8. 
9. 🔹 PASO 2: Identificar términos
10.    • Lineal: n
11.    • Lineal: n
...
**Ecuación:** `E[T]=q*(7*(n+1)/2+10)+(1-q)*(7*n+5)=(7*q*n)/2+(7*q)/2+10*q+7*n-7*q*n+5-5*q=(7*n)/2-(7*q*n)/2+(7*q)/2+5*q+5`
**Método:** AnalizadorDirecto

**Pasos:**
1. 📝 Expresión: E[T] = q * ( 7 * ( n + 1 ) / 2 + 10 ) + ( 1 - q ) * ( 7 * n + 5 ) = ( 7 * q * n ) / 2 + ( 7 * q ) / 2 + 10 * q + 7 * n - 7 * q * n + 5 - 5 * q = ( 7 * n ) / 2 - ( 7 * q * n ) / 2 + ( 7 * q ) / 2 + 5 * q + 5
2. 
3. 🔹 ANÁLISIS DE EXPRESIÓN DIRECTA
4.    Esta NO es una recurrencia, es una fórmula cerrada
5. 
6. 🔹 PASO 1: Analizar expresión
7.    Expresión: q * ( 7 * ( n + 1 ) / 2 + 10 ) + ( 1 - q ) * ( 7 * n + 5 )
8. 
9. 🔹 PASO 2: Identificar términos
10.    • Lineal: n
11.    • Lineal: 7*n
12. 
13. 🔹 PASO 3: Determinar término dominante
14.    Jerarquía: constante < log(n) < n < n·log(n) < n² < n³ < 2ⁿ < n!
15.    Término dominante: lineal
16. 
17. ✅ SOLUCIÓN: T(n) = n
...
**Duración total:** 111.87 segundos
